...
# Cloud Edition 표시
st.sidebar.info("☁️ Cloud Edition | 인덱싱: Google Colab")
...
# ============================================================
# ============================================================
# Inbox 관리 페이지 (Cloud Edition - Metadata Editor)
# ============================================================
elif page == "📁 Inbox":
    st.markdown("""
        <div style="padding-bottom: 25px;">
            <h1 style="color: #2D3748; font-weight: 800; letter-spacing: -0.5px; margin-bottom: 0;">📁 Inbox 관리</h1>
            <p style="color: #718096; font-size: 1.1em; font-weight: 400;">Cloud Indexing 전에 파일 메타데이터(제목, 저자, 페이지)를 정밀 보정합니다.</p>
        </div>
    """, unsafe_allow_html=True)
                ...
                with st.form("meta_form"):
                    ...
                    if st.form_submit_button("💾 설정 저장 (Save JSON)"):
                        ...
                        try:
                            ...
                            st.balloons()
                            st.success(f"저장 완료! 이제 Colab에서 이 설정을 사용합니다.\n경로: `{save_path.name}`")
                            st.rerun()
                        except Exception as e:
                            st.error(f"저장 실패: {e}")
