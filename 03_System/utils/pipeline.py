...
class ProcessingPipeline:
    ...
    def run_processor(self, file_path: Path, chunk_size: int = 800, overlap: int = 200) -> bool:
        """
    ...
    def process_file(self, file_path: Path, chunk_size: int = 800, overlap: int = 200) -> Dict[str, Any]:
        """
        단일 파일 처리 (전체 파이프라인)
    ...
    def process_inbox(self, chunk_size: int = 800, overlap: int = 200) -> Dict[str, Any]:
        """
        inbox 폴더 전체 처리
        ...
        # 처리할 파일 목록 (재귀 검색 지원)
        files = list(self.inbox_dir.rglob("*.pdf")) + list(self.inbox_dir.rglob("*.txt")) + list(self.inbox_dir.rglob("*.epub"))
