...
from pipeline.chunker import TheologyChunker
...
class ProjectV4Processor:
    ...
    def process_file_with_metadata(self, file_path: Path, parsed_meta) -> List[Document]:
        """Process file with parsed metadata."""
        logger.info(f"📄 Processing: {file_path.name} (Type: {parsed_meta.doc_type})")
        
        chunk_size = parsed_meta.chunk_size
        overlap = parsed_meta.chunk_overlap
        
        # Build base metadata from parsed info
        # Note: ChromaDB only supports str, int, float, bool - convert lists to strings
        base_meta = {
            "source": file_path.name,
